            ...
            <div class="row">
                ...
                <div class="card">
                    <div class="card-header py-3">
                        <div style="font-size: 18px; font-weight: 600">Angkatan: {{ $angkatan->angkatan }}</div>
                    </div>
                                ...
                                <table id="kt_table_data"
                                    ...
                                    <thead class="text-center">
                                        <tr class="fw-bolder fs-6 text-gray-800">
                                            ...
                                            <th>Status</th>
                                            <th>Transkrip</th>
                                            <th>Surat Kelakuan Baik</th>
                                            <th>Surat Pernyataan Kesiapan</th>
                                            <th>Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            window.location.href = '/admin/mahasiswa-add';
        })

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            window.location.href = '/admin/mahasiswa-edit/' + $(this).attr('data-uuid');
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/mahasiswa-delete/' + $(this).attr('data-uuid');
        ...
        $(document).on('click', '.button-convir', function(e) {
            e.preventDefault();
            control.submitFormMultipartData('/admin/mahasiswa-konfirmasi/' + $(this).attr(
                    'data-uuid'),
                'Update',
                'Mahasiswa', 'POST');
        })
            ...
            $('#kt_table_data').DataTable({
                ...
                columns: [{
                    ...
                    render: function(data, type, row, meta) {
                        let result;
                        result =
                            `<a class="btn btn-outline btn-outline-dashed ${data == 'Terkonfirmasi' ? 'btn-outline-success btn-active-light-success' : 'btn-outline-danger btn-active-light-danger'} p-2 py-1">
                    ...
                    render: function(data, type, row, meta) {
                        let result;
                        result =
                            `<a href="{{ asset('/public/mahasiswa/${data}') }}" target="_blank" class="btn btn-outline btn-outline-dashed btn-outline-danger btn-active-light-danger p-2 py-1">
