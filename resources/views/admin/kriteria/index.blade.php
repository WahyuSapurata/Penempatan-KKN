            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <small class="text-danger nama_kriteria_error"></small>
                    </div>
                    ...
                    <div class="mb-10">
                        ...
                        <select name="jenis" class="form-select" data-control="select2" id="from_select"
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Kriteria');
        })
        ...
        $(document).on('submit', ".form-data", async function(e) {
            ...
            const totalBobot = await getBobot();
            const currentBobot = parseFloat($('#bobot').val()) || 0;

            if (type === 'add') {
                if ((totalBobot + currentBobot) > 100) {
                    $('.bobot_error').text('Total bobot melebihi 100, tidak bisa menambahkan!');
                    return;
                }
            } else {
                const oldBobot = parseFloat($('#bobot').attr('data-old')) || 0;
                const newTotal = totalBobot - oldBobot + currentBobot;

                if (newTotal > 100) {
                    $('.bobot_error').text(`Perubahan bobot melebihi 100! Total menjadi ${newTotal}`);
                    return;
                }
            }
            ...
            if (type === 'add') {
                control.submitFormMultipartData('/admin/kriteria-add', 'Tambah', 'Kriteria', 'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/kriteria-edit/' + uuid, 'Update', 'Kriteria', 'POST');
            }
        ...
        $(document).on('click', '.button-update', function(e) {
            ...
            let bobot = $(this).attr('data-bobot');
            let url = '/admin/kriteria-show/' + uuid;

            control.overlay_form('Update', 'Kriteria', url);

            setTimeout(() => {
                $('#bobot').attr('data-old', bobot);
            }, 500);
        });

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/kriteria-delete/' + $(this).attr('data-uuid');
            let label = $(this).attr('data-label');
            control.ajaxDelete(url, label, getBobot);
        })

        $(document).on('click', '.button-sub-kriteria', function(e) {
            e.preventDefault();
            window.location.href = '/admin/sub-kriteria/' + $(this).attr('data-uuid');
        })
                ...
                columns: [{
                    ...
                    render: function(data, type, row, meta) {
                        if (!data || data.length === 0) {
                            return 'Belum ada sub kriteria';
                        }

                        let list = '<ul style="padding-left: 20px; margin: 0;">';
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        <div class="d-flex gap-2" style="flex-flow: wrap;">
                            <a href="javascript:;" type="button" data-uuid="${data}" data-bobot="${full.bobot}" data-kt-drawer-show="true" data-kt-drawer-target="#side_form" class="btn btn-warning button-update btn-icon btn-sm">
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Kriteria" class="btn btn-danger button-delete btn-icon btn-sm ${['Jenis Kelamin', 'Fakultas', 'Jurusan'].includes(full.nama_kriteria) ? 'disabled' : ''}">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                             ...
                             <a href="javascript:;" type="button" data-uuid="${data}" class="btn btn-success button-sub-kriteria btn-sm">
                                Tambah Sub Kriteria
                            </a>
                             </div>
                        `;
            ...
            try {
                ...
                if (res.success === true) {
                    ...
                    <div class="d-flex justify-content-center">
                        <div class="alert bg-light-success border border-success d-flex justify-content-center py-2 m-0 gap-4 align-items-center" style="max-width: max-content">
                ...
                `);
                        ...
                        $('#alert-bobot').html(`
                    <div class="d-flex justify-content-center">
                        <div class="alert bg-light-warning border border-warning d-flex justify-content-center py-2 m-0 gap-4 align-items-center" style="max-width: max-content">
                ...
                `);
                    ...
                    return hasil; // <--- penting untuk pengecekan saat submit
        ...
        $(function() {
            initDatatable();
            getBobot();
            control.push_select3(data, '#from_select');
        });
    </script>
@endsection
